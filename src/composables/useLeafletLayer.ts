...
import { inject, nextTick, onBeforeUnmount, shallowRef, type ShallowRef, type useAttrs } from 'vue';
...
export const useLeafletLayer = <TLayer extends Layer>(
  ...
  nextTick(() => {
    ...
    if (options.attrs && options.events) {
      unbindEvents = bindLeafletEventsFromAttrs(layer, options.attrs, options.events);
    }
